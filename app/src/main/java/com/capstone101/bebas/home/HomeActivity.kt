...
import com.capstone101.bebas.databinding.ActivityHomeBinding
...
class HomeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bind.recordHome.setOnClickListener {
            count++
            when (count) {
                3 -> {
        ...
        viewModel.getUser.observe(this) {
            if (it != null) {
                MapVal.user = it
                bind.test1.text = it.toString()
            } else bind.test1.text = "ERROR COK"
        }
    }
    ...
    private fun recording() {
        ...
        isRecording = true
        bind.recordHome.isEnabled = false
        checkFolder()

        bind.recordHome.setImageResource(R.drawable.recording)
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            ...
            bind.recordHome.isEnabled = true
            count = 0
            bind.recordHome.setImageResource(R.drawable.record)
        }, 10000)
    }
